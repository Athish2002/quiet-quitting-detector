...
- **Fuzzy Alias Mapping:** Maps raw data keys from diverse sources (SQLite DB columns, S3/local-bucket objects, raw CSV rows, uploaded files, webhook payloads, text prompts) to standardized keys. For example, columns like `hours_worked`, `hours`, or `weekly hours` are mapped seamlessly to `weekly_hours`.
- **Six Live Ingest Paths:** Raw CSV paste, file upload, natural language prompt extraction, a real local SQLite database, a real local bucket folder (with optional live AWS S3), and a JSON webhook are all genuinely functional -- none are mocked. See `src/data_layer/sql_store.py` and `src/data_layer/s3_store.py`.
