...
@router.get("/history", summary="Ingestion and pipeline event log")
...
@router.post("/history/clear", summary="Clear the event log")
...
@router.get("/report/raw", summary="Main cohort report")
def get_raw_report() -> FileResponse:
    return _serve(MAIN_REPORT, "text/plain", "Engagement report file not found.")


@router.get("/report/realtime", summary="Realtime cohort report")
...
@router.get("/report/threat-model", summary="STRIDE threat model")
